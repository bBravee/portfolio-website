...
function ProjectDetails({ projectsData }) {
  ...
  let project = projectsData.find((item) => item.id === parseInt(id));

  if (!project) {
    project = projectsData.find((item) => item.id === parseInt(id));
  }
      ...
      <div className="d-flex flex-column justify-content-center align-items-center">
        <div className="d-flex justify-content-center flex-row pt-5">
          <a href={project.repoUrl}>
            <Button className="pd-button" size="lg">
              {" "}
